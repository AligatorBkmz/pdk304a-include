...
TBool aScrollBarIsVisible = EFalse;
...
TBool CMmGrid::HandleScrollbarVisibilityChangeL()
    ...
    TBool iEditMode = widget->IsEditMode();
    
    TBool iScrollsActive = ScrollBarFrame()->VerticalScrollBar()->IsVisible()
...
//            }
    ...
    ScrollBarFrame()->VerticalScrollBar()->MakeVisible( iScrollsActive );
    iMmDrawer->SetScrollbarVisibilityL( iScrollsActive );       
...
void CMmGrid::HandleRedrawTimerEventL()
    ...
    if ( iSkippedScrollbarEventsCount )
        {
        ProcessScrollEventL( ScrollBarFrame()->VerticalScrollBar(), EEikScrollThumbDragVert );
        }
...
TInt CMmGrid::RedrawTimerCallback( TAny* aPtr )
    ...
    // Do not bother returning a meaningful error error code, CPeriodic will ignore it
    // anyway.
    return 0;
    }
